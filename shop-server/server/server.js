...
app.use('/api/cart', cart); //при открытии url с адресом '/api/cart' запускаем модуль cart (строка 3 здесь)
app.use('/', express.static('public')); //при открытии главной страницы сообщаем открыть статическую страницу
...
app.get('/api/products', (req, res) => { //по умолчанию возвращаем data - наши товары
  ...
  //readFile - считываем данные с файла products.json
    if(err){
...
// server.js - это роутер - это файл, который обрабатывает все запросы
// 4 основных запроса:
...
//const c = require('./func')  - импорт локального модуля, где  ./ - текущая папка, func - имя файла func.js
...
//на собеседовании могут попросить сделать онлайн-аукцион, необходимо будет применить socket
...
cartRouter.js вызывает модуль (функцию) Handler.js, в котором есть объект actions (с методами: добавление, изменение, удаление)
